/**************************************************************************************************************
 ...
 Beschreibung: Konfiguriert mehrere NiboBee-Bluetooth Adapter mit 
               fortlaufendem Namen: Robby01, Robby02, ...
               Einstellung der Baudrate für die Verbindung Host<->NiboBlue
Rahm, 14.1.18
*************************************************************************************************************/
#include "controller.h"

#define OK     _PORTB_,5
#define Plus   _PORTB_,4
#define Minus  _PORTB_,3
#define S1     _PORTB_,2

void SendAT_Kommando(uint8_t *pChar);
void get_baudrate(void);

uint8_t pin[] = "1234";         // Standard BT-PIN
uint8_t name[] = "Robby";       // Standard Namensprefix

volatile uint16_t time16;
volatile int8_t baud = 3;       //Standard-Baudrate 9600 Bd
	...
	while(1)
	{
      ...
      count++;
			while(bit_read(OK))
			{
				...
				else if(!bit_read(Minus))
				{
					delay_ms(40);
					if(count>1)	  count--;
					while(!bit_read(Minus));
					delay_ms(40);
				}
				lcd_setcursor(1,12);
				lcd_dd(count);
